...
| `rmm_list_devices` | READ | Paginated device list; routes to site endpoint when siteUid provided |
...
| `rmm_list_alerts` | READ | Alerts with flexible routing by status, site, or device |
...
| `rmm_run_job` | WRITE | Runs a component on a device; confirm before calling |
...
| `rmm_resolve_alert` | medium | Resolves (closes) an alert; irreversible |
...
| `rmm_get_job_status` | READ | Per-device job results; includes stdout if job completed |

## Tier 2 — Audit

| Tool | Class | Notes |
|------|-------|-------|
| `rmm_get_device_audit` | READ | Hardware/software audit; auto-routes by device class |
| `rmm_get_device_software` | READ | Installed software list |
| `rmm_get_device_audit_by_mac` | READ | Audit lookup by MAC address |
| `rmm_list_patches` | READ | Patch list for a device or site |

## Tier 2 — Activity

| Tool | Class | Notes |
|------|-------|-------|
| `rmm_list_activity_logs` | READ | Account activity log |

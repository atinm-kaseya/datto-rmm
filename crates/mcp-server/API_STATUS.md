...
|------|-------------|--------------|
| `rmm_get_account_dashboard` | Account-wide overview with metrics | Multi-site aggregation, offline device tracking, alert summaries |
| `rmm_find_sites_with_issues` | Identify problematic sites | Severity ranking, multi-factor scoring, parallel queries |
| `rmm_search_devices` | Flexible device search with filters | Pattern matching, status filters, rich formatting |
| `rmm_get_site_health` | Comprehensive site health check | **Enhanced:** Variables, alert type grouping, device details, recommendations with code examples |
| `rmm_list_site_devices` | Browse devices with rich formatting | **Enhanced:** Type filter, alert counts, 3 sort options (name/alerts/last_seen), IP addresses, last seen times |
| `rmm_get_site_alerts` | Site alert overview with grouping | **Enhanced:** Group by device or type, priority icons, device name lookup, alert categorization |
| `rmm_get_alert_summary` | Alert aggregation and categorization | Cross-site analysis, pattern detection, priority grouping |

**Enhancement Details:**

- **rmm_get_site_health** now includes:
...
- **rmm_list_site_devices** now includes:
...
- **rmm_get_site_alerts** now includes:
...
|------|---------|----------------|
| `rmm_get_account_analytics` | Historical trends and capacity planning | Framework with guidance text |
| `rmm_diagnose_device_issue` | Multi-source device diagnostics | Framework with diagnostic prompts |
| `rmm_investigate_alert` | Pattern detection and remediation | Framework with investigation steps |
...
|------|-------------|----------|
| `rmm_run_site_component` | Execute components on site devices | Dry-run support, device selection |
| `rmm_bulk_update_site_devices` | Bulk device property updates | Dry-run preview, safety checks |
| `rmm_get_device_health` | Device diagnostics and health | Multi-source data aggregation |
...
|------|------------|----------|
| `rmm_get_account` | `get_account` | GET `/v2/account` |
...
| `rmm_list_account_variables` | `list_account_variables` | GET `/v2/account/variables` |
| `rmm_list_users` | `list_users` | GET `/v2/user` |

### Site Operations (9 tools)

| Tool | API Method | Endpoint |
|------|------------|----------|
| `rmm_get_site` | `get_site` | GET `/v2/site/{siteUid}` |
| `rmm_get_site_devices` | `list_site_devices` | GET `/v2/site/{siteUid}/devices` |
| `rmm_get_site_alerts` | `list_site_open_alerts` | GET `/v2/site/{siteUid}/alerts/open` |
| `rmm_get_site-resolved-alerts` | `list_site_resolved_alerts` | GET `/v2/site/{siteUid}/alerts/resolved` |
| `rmm_get_site_settings` | `get_site_settings` | GET `/v2/site/{siteUid}/settings` |
| `rmm_get_site-variables` | `list_site_variables` | GET `/v2/site/{siteUid}/variables` |
| `rmm_create_site` | `create_site` | PUT `/v2/site` |
| `rmm_update_site` | `update_site` | POST `/v2/site/{siteUid}` |
| `rmm_get_site-filters` | `list_site_filters` | GET `/v2/site/{siteUid}/filters` |

### Device Operations (9 tools)

| Tool | API Method | Endpoint |
|------|------------|----------|
| `rmm_get_device` | `get_device` | GET `/v2/device/{deviceUid}` |
| `rmm_get_device_by_id` | `get_device_by_id` | GET `/v2/device/id/{deviceId}` |
| `rmm_get_device_by_mac` | `get_device_by_mac` | GET `/v2/device/macAddress/{macAddress}` |
| `rmm_get_device-alerts` | `list_device_open_alerts` | GET `/v2/device/{deviceUid}/alerts/open` |
| `rmm_get_device-resolved-alerts` | `list_device_resolved_alerts` | GET `/v2/device/{deviceUid}/alerts/resolved` |
| `rmm_move_device-to-site` | `move_device` | POST `/v2/device/{deviceUid}/site/{siteUid}` |
| `rmm_set_device_udf` | `set_device_udf` | PUT `/v2/device/{deviceUid}/udf` |
| `rmm_set_device_warranty` | `set_device_warranty` | PUT `/v2/device/{deviceUid}/warranty` |
| `rmm_create_quick_job` | `create_quick_job` | PUT `/v2/device/{deviceUid}/quickjob` |

### Alert Operations (2 tools)

| Tool | API Method | Endpoint |
|------|------------|----------|
| `rmm_get_alert` | `get_alert` | GET `/v2/alert/{alertUid}` |
| `rmm_resolve_alert` | `resolve_alert` | POST `/v2/alert/{alertUid}/resolve` |

### Job Operations (5 tools)

| Tool | API Method | Endpoint |
|------|------------|----------|
| `rmm_get_job` | `get_job` | GET `/v2/job/{jobUid}` |
| `rmm_get_job_results` | `get_job_results` | GET `/v2/job/{jobUid}/results` |
| `rmm_get_job_components` | `get_job_components` | GET `/v2/job/{jobUid}/components` |
| `rmm_get_job_stdout` | `get_job_stdout` | GET `/v2/job/{jobUid}/results/{deviceUid}/stdout` |
| `rmm_get_job_stderr` | `get_job_stderr` | GET `/v2/job/{jobUid}/results/{deviceUid}/stderr` |

### Audit Operations (5 tools)

| Tool | API Method | Endpoint |
|------|------------|----------|
| `rmm_get_device_audit` | `get_device_audit` | GET `/v2/audit/device/{deviceUid}` |
| `rmm_get_device_software` | `get_device_software` | GET `/v2/audit/device/{deviceUid}/software` |
| `rmm_get_device_audit_by_mac` | `get_device_audit_by_mac` | GET `/v2/audit/device/macAddress/{macAddress}` |
| `rmm_get_esxi_audit` | `get_esxi_audit` | GET `/v2/audit/esxi/{deviceUid}` |
| `rmm_get_printer_audit` | `get_printer_audit` | GET `/v2/audit/printer/{deviceUid}` |
...
| Tool | API Method | Endpoint |
|------|------------|----------|
| `rmm_get_system_status` | `get_system_status` | GET `/v2/system/status` |
| `rmm_get_rate_limit-info` | `get_rate_limit_info` | GET `/v2/system/request_rate` |
| `rmm_get_pagination_config` | `get_pagination_config` | GET `/v2/system/pagination` |
...
| Tool | API Method | Endpoint |
|------|------------|----------|
| `rmm_create_account_variable` | `create_account_variable` | PUT `/v2/account/variable` |
| `rmm_update_account_variable` | `update_account_variable` | POST `/v2/account/variable/{varId}` |
| `rmm_delete_account_variable` | `delete_account_variable` | DELETE `/v2/account/variable/{varId}` |
| `rmm_create_site_variable` | `create_site_variable` | PUT `/v2/site/{siteUid}/variable` |
| `rmm_update_site_variable` | `update_site_variable` | POST `/v2/site/{siteUid}/variable/{varId}` |
| `rmm_delete_site_variable` | `delete_site_variable` | DELETE `/v2/site/{siteUid}/variable/{varId}` |
| `rmm_update_site_proxy` | `update_site_proxy` | POST `/v2/site/{siteUid}/settings/proxy` |
| `rmm_delete_site_proxy` | `delete_site_proxy` | DELETE `/v2/site/{siteUid}/settings/proxy` |
